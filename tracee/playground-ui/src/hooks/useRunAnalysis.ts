...
import { computeSimilarity, SimilarityResult } from '../utils/cosineSimilarity';
  ...
  const { analyzed, consensus } = useMemo(() => {
    ...
    const baseRuns: AnalyzedRun[] = results.map((run, i) => {
      ...
      return {
        ...
        classification: 'conforming' as RunClassification,
        deviations: [], validationErrors,
      };
    });
    ...
    const finalRuns = consensusResult
      ? baseRuns.map(run => {
          ...
          return { ...run, deviations, classification: classifyRun(deviations) };
        })
      : baseRuns;
